...
cols.with.ab.zeros = function(my.mat, a=0, b=Inf) {
  zeros.per.column <- colSums(mat == 0) ## changed to makes colSum how many times 0 was in the column instead of 1. 
  i.to.keep = a <= zeros.per.column ##removed && statement
  return(my.mat[,i.to.keep]) ## moved comma to keep columns instead of rows.
  ## This was as close as I could get before I stopped. Didn't think I could spare anymore time before it was due
  ## Since I still had to work on the midterm and HW8.
}
...
list.extractor = function(my.list, i.to.keep=NULL, i.to.remove=NULL) {
  if (is.null(i.to.keep) == FALSE) {
    L = my.list[i.to.keep] ##Removed double brackets to get rid of indexing error
  return(L) ##added return functions for each separate if statement to returns correct values to L
            ##this also makes it so the first one takes precedence as the return function ends the
            ##list.extractor function
  }
